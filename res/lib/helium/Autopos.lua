...
function Autopos.vert:X()
	...
	return prev and prev.outer:X()
		or (self.node.parent and self.node.parent.inner:X() or 0)
end
function Autopos.vert:Y()
	local prev = self.node:sibling(-1)
	return prev and prev.outer:Y() + prev.outer:H()
...
function Autopos.hor:X()
	local prev = self.node:sibling(-1)
	return prev and prev.outer:X() + prev.outer:W()
		or (self.node.parent and self.node.parent.inner:X() or 0)
end
function Autopos.hor:Y()
	local prev = self.node:sibling(-1)
	return prev and prev.outer:Y()
		or (self.node.parent and self.node.parent.inner:Y() or 0)
end
